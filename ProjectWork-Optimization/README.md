...
```
  ...
  **Но теперь проверим производительность скрипта обновления данных:**
  ...
  Как видно Execution Time увеличился до 1343.255 ms (было 712.384 ms), создание индексов, которые ускоряют поиск данных, негатвно повлияло на процесс изменения данных. 

  Но почему именно в нашем случае замедлилися скипт обновления данных? Данный скрипт обновляет поле ``rating``, на которе я ранее создал составной индекс. В запросах массового изменения данных структура индексов может продлевает отработку поскольку требует дополнительного времени на синхронизацию данных таблицы с индексами, для потдержания данных в индексах в актульаном состоянии.
  
  Для достижения наиболее оптимальной структуры, попробую удалить созданный ранее составной индекс, и понаблюдать как это повлияет на производительность обох запросов. 

  ``DROP INDEX IF EXISTS idx_user_scores_user_id_rating;``

  **Резултат скрипта обновления данных:**
