...
```
  ...
  **Контрольный замер производительности с EXPLAIN ANALYZE:**
  ...
  Одныко в данном случае сам запрос не является оптимальным, таккак составлен не лучшим образом. А имеено данный запрос использует устаревший синтаксис с WHERE вместо JOIN, что хуже оптимизируется.
